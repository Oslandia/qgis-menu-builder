...
class Menus():
 ...
 def initGui(self):
  ...
  if fichierMenus=="": # 1er demarrage du plugin
    # Menus.xml ira à la base du profil Qgis (ex: USERPROFILE\.qgis2\Menus.xml )
    ...
    if not QFile.exists( fichierMenus ): # cas où Qgis a été déployé avec ce plugin dans un dossier différent
 ...
 def fenetreParam(self):
  ...
  while True:
    ...
    if PluginPath == fileDir:
      msgBox.setText( u"Désolé, vous ne pouvez pas placer votre fichier dans le dossier du plugin." )
 ...
 def fenetreAide(self, modal=False):
  ...
  form.setWindowTitle(u"Mode d'emploi de l'extension : Créer ses propres menus")
 ...
 def traiterAction(self, elm, menu):
  ...
  if nom == '':  return # l'action a été mal rédigée
  ...
  newaction.setWhatsThis( val )  # astuce pour faire récupérer le nom du fichier dans la fonction affiche...
 ...
 def afficheRaster(self):
  ...
  if QGis.QGIS_VERSION_INT<20400: # pour qgis jusqu'à 2.2.x
    ...
    QgsMapLayerRegistry.instance().addMapLayer(macouche,False) #prête à être utilisée mais pas encore affichée
  ...
  def lireGroupe22( domGrp, treeGrp ): #valid jusqu'à qgis 2.2
    ...
    for N in range(elems.length()-1,-1,-1): # parcourir les couches du groupe en partant de la dernière
      ...
      if ele.tagName()=="legendgroup":
        ...
        new = legendTree.currentItem() #! mais l'élément actif n'est pas toujours le groupe qu'on vient d'ajouter...
      ...
      for i in range(maps.length()): # rechercher dans les couches du projet celle qui a layId
        ...
        new = legendTree.currentItem() #! mais l'élément actif n'est pas toujours la couche qu'on vient d'ajouter...
        ...
        if ele.attribute("checked")=="Qt::Unchecked": new.setCheckState(0,Qt.Unchecked) #retablie l'etat: affiché ou non
  ...
  def lireGroupe( domGrp, treeGrp ):
    ...
    for N in range(elems.length()-1,-1,-1): # parcourir les couches du groupe en partant de la dernière
      ...
      for i in range(maps.length()): # rechercher dans les couches du projet celle qui a layId
        ...
        nomCouche = maps.item(i).firstChildElement("layername").toElement().text()
        QgsProject.instance().read( maps.item(i) ) # ajouter la couche
        new = legendTree.currentLayer() #! mais l'élément actif n'est pas toujours la couche qu'on vient d'ajouter...
        if not new or not new.originalName()==nomCouche: # l'element actif dans la legende est peut-etre le dossier parent:
          ...
          if not new:  break # pas trouvé
        else: new = legendTree.currentNode()
        new.setExpanded( False )
        if ele.attribute("checked")=="Qt::Unchecked": new.setVisible(Qt.Unchecked) #retablie l'etat: affiché ou non
 ...
 def afficheWMS(self):
  ...
  if len(lUri) == 1 : # c'est un simple lien web --> afficher dans un navigateur
    ...
    return
  if len(lUri) != 4 : return  # WMS mal défini
